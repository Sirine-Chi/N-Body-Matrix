...
class ForceHandler:
    ...
    @staticmethod
    def gravity_force_ent(ent1: int, ent2: int):
        m1 = esper.try_component(ent1, Mass)
        m2 = esper.try_component(ent2, Mass)
        r1 = esper.try_component(ent1, Position)[-1]
        r2 = esper.try_component(ent1, Position)[-1]
    ...
    def hooke_force(r1: l.Array, r2:l.Array):
        st_lenght = 1.0
        return mymath.K * ( (r1 - r2)/(r1 - r2).l.scal() - st_lenght)

    @staticmethod
    def hooke_force_ent(ent1: int, ent2: int):
        r1 = esper.try_component(ent1, Position)[-1]
        r2 = esper.try_component(ent2, Position)[-1]
        st_lenght = 1.0
        return 0 * ( (r1 - r2)/(r1 - r2).l.scal() - st_lenght)
...
class ForceProcessor(esper.Processor):
    ...
    def process(self):
        ...
        for force_id, force in self.all_funcs.items():
            for ent2 in esper.get_component(Force):
        ...
        for ent, (acc, vel, pos) in esper.get_components(Acceleration, Velocity, Position):
            vel.append(vel[-1] + acc*self.timestep)
            pos.append(pos[-1] + vel*self.timestep)
...
for i in range(1, 10):
    ...
    esper.add_component(particle, Force(l.Array([0.0, 0.0, 0.0]))) # FIXME dimensional independent INIT'ion
    ...
    print(f"INIT: {esper.try_components(particle, Name, Mass, Position)}")

# --- --- --- --- --- FIRST FORCE COLLECTION

for force_id, force in ForceProcessor.all_funcs.items():
    for ent2 in esper.get_component(Force):
        for ent1, (f1) in esper.get_component(Force):
            if ent1 != ent2:
                if isinstance(ent1, int) and isinstance(force_id, str): # если тело2 действует на
