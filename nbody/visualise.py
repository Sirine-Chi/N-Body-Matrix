...
def vis_N_2D(system, inum, delta_cur, directory):
    ...
    plt.savefig(
        directory + "/" + str(inum) + "_delta=" + str(delta_cur) + ".png", dpi=200
    )
    # plt.show()
    return plt
    plt.clf()
...
def vis_N_3D(galaxy):
    ...
    for dot in galaxy:
        ax.plot(
            galaxy[dot.i].makeXY()[0],
            galaxy[dot.i].makeXY()[1],
            galaxy[dot.i].t,
            alpha=0.4,
            marker=" ",
            c="g",
...
def vis_N_anim(galaxy, enn):
    ...
    galanim = anima.FuncAnimation(
        fig, animate, frames=galaxy[0].t, interval=100, repeat=False
    )
    return galanim
